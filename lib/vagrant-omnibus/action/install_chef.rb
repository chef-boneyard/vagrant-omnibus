    ...
    module Action
      ...
      class InstallChef
        ...
        def initialize(app, env)
          ...
          if @is_windows_guest && !ENV['OMNIBUS_INSTALL_URL']
              # create URL to get desired chef version
              # adapted from https://github.com/opscode/knife-windows/blob/master/lib/chef/knife/bootstrap/windows-chef-client-msi.erb
              version = @machine.config.omnibus.chef_version
              windows_version = /Version (?<major_minor>\d+\.\d+)\.(?<build>\d+)/.match(`cmd /c ver`)
              ...
              # detect architecture
              windows_arch = ENV["PROCESSOR_ARCHITEW6432"] || ENV["PROCESSOR_ARCHITECTURE"]
              ...
              @install_sh = "https://www.opscode.com/chef/download?p=windows&pv=#{machine_os}&m=#{machine_arch}&v=#{version}"
              @local_install_name = "install.bat"
              @logger.info("Chef windows installer will be taken from: #{@install_sh}")
          end
        ...
        # Uploads install.sh or install.bat from Host's Vagrant TMP directory to guest
        # and executes.
        def install(version, env)
          ...
          @machine.communicate.tap do |comm|
            ...
            if @is_windows_guest
              ...
              install_cmd = "bash #{@local_install_name} -v #{shell_escaped_version} 2>&1"
            end
        ...
        # Fetches install.sh or creates install.bat file to the Host's Vagrant TMP directory.
        #
        # Mostly lifted from:
        #
        ...
        def fetch_install_sh(env)
          ...
          @logger.info("Downloading #{@local_install_name} to: #{@install_sh_temp_path}")
          ...
          # Download the install.sh or create install.bat file to a temporary path. We store
